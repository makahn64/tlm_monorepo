...
import {ValueDescription} from "../../types/commonTypes";
  ...
  const renderBoxes = () => {
    return allChoices.map(c =>
      <Form.Check
        ...
        key={c.value}
        label={c.description}
        onChange={(ev: React.ChangeEvent<HTMLInputElement>) => checkChanged(c.value, ev.target.checked)}
        checked={trueValues.includes(c.value)}
      />);
  }
